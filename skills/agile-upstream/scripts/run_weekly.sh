#!/bin/bash
# run_weekly.sh — Orquestra a rotina AGENDADA da Visão de Épicos Agarrados de
# ponta a ponta (Azure DevOps → PDF → Google Drive → Google Chat), pensada para
# rodar sem supervisão via launchd. Em qualquer falha, posta um alerta no Chat e
# sai com código != 0 (sem postar relatório incompleto).
#
# Para um relatório AVULSO (sob demanda, SEM Drive/Chat) rode apenas:
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/agile-upstream_$(date +%Y-%m-%d).log"
...
notify_failure() {
  ...
  if [ -n "${GCHAT_WEBHOOK_URL:-}" ]; then
    curl -s -X POST -H 'Content-Type: application/json' \
      -d "{\"text\":\"⚠️ *Rotina de épicos agarrados falhou* na etapa: ${step}. Log: ${LOG_FILE}\"}" \
