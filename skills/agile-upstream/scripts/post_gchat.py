...
def build_message(link: str, s: dict) -> str:
    ...
    return (
        ...
        f"🟣 Possível descarte: *{s.get('descarte','?')}*\n"
        f"🟣 Funil possível descarte (idade > 180d): *{s.get('funil_descarte','?')}*\n"
        f"\n---\n\n"
        f"*POs, atenção ao seu time!* Cada seção do PDF é o seu time (AreaPath). "
        f"Priorize os épicos 🔴 *Agarrados* e avalie *descartar* os 🟣 "
        f"*Possível descarte* (em fluxo e no funil) para manter o flow saudável.\n"
        f"\n"
